...
class Collections:
    def __init__(self):
        ...
        collections_query = g.query(f"""PREFIX dcterms: <http://purl.org/dc/terms/>
                             ...
                             SELECT ?fc ?identifier ?title ?description
                              ...
                              """)
        collections_query = [{str(k): v for k, v in i.items()} for i in collections_query.bindings]
        fc = [str(i["fc"]) for i in collections_query]
        descriptions = [i["description"] if "description" in i.keys() else None for i in collections_query]
        identifiers = [str(i["identifier"]) if "identifier" in i.keys() else None for i in collections_query]
